...
이 시스템은 루프를 돌면서 각 단계마다 새로운 동작을 실행합니다(이 동작에는 함수 형태로 정의된 사전 지정 *도구*를 호출하는 것이 포함될 수 있습니다). 그런 다음 관찰 결과를 통해 주어진 작업을 해결하기에 충분한 상태에 도달했다고 판단될 때까지 반복합니다.
...
에이전트는 LLM이 애플리케이션의 워크플로우를 결정해야 할 때 유용합니다. 하지만 과잉일 때가 종종 발생합니다. 질문점은 이것입니다 : “지금 당면한 과제를 효율적으로 해결하기 위해서는 **워크플로우의 유연성이** 필요할까?” 미리 지정한 워크플로우에 자주 한계가 보인다면, 더 큰 유연성이 필요하다는 뜻입니다. 예시를 들어볼게요. 당신이 서핑 여행 웹사이트에서 고객 요청을 처리 앱을 만든다고 해봅시다.

사용자의 선택에 따라 요청이 두 가지 경우 중 하나에 속할 것이라는 걸 미리 알 수 있고, 각 경우마다 미리 정해둔 워크플로가 있다고 가정해 봅시다.

1. 여행 관련 정보를 알고 싶다 ⇒ 지식베이스를 검색할 수 있는 검색창 제공
2. 영업팀과 이야기하고 싶다 ⇒ 문의 양식을 작성하도록 안내

만약 이런 결정론적 워크플로우가 모든 요청을 처리할 수 있다면, 다 코드로 짜는 게 좋습니다! 

이렇게 하면 LLM 같은 예측 불가능한 요소가 개입해 오류를 만들 위험 없이 100% 안정적인 시스템을 얻을 수 있습니다. 단순성과 견고함을 위해서는 에이전트 같은 행위자적 기능을 사용하지 않는 편이 권장됩니다.

하지만 워크플로를 미리 그렇게 잘 규정할 수 없는 경우는 어떨까요?
...
즉, 미리 정해둔 워크플로우에 자주 한계가 있다면, 더 많은 유연성이 필요하다는 뜻입니다.
...
최근까지 프로그램은 미리 정해진 워크플로우에 제한되어, 복잡성을 처리하기 위해 if/else 조건문을 잔뜩 쌓는 방식에만 머물러 있었습니다. 이런 프로그램들은 “이 숫자들의 합을 계산하라”거나 “이 그래프에서 최단 경로를 찾아라” 같은 극도로 좁은 과제에 집중했습니다. 하지만 위의 여행 예시 같은 실생활 과제들은 미리 정해둔 워크플로에 잘 들어맞지 않습니다. 이런 관점에서 에이전틱 시스템은 프로그램에게 실생활의 다양한 과제를 연결해주는 열쇠입니다.
...
우리는 여러 최신 모델을 활용해 [**`CodeAgent`**](https://huggingface.co/docs/smolagents/reference/agents#smolagents.CodeAgent) 인스턴스를 만들었고, 다양한 벤치마크에서 질문을 모아 여러 유형의 과제를 제공하는 [**이 벤치마크**](https://huggingface.co/datasets/m-ric/agents_medium_benchmark_2)로 비교했습니다.
